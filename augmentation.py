...
for file in img_files:
    if 'aug' not in file:
        ...
        cv2.imwrite("v4/origin/img/"+file, image)
        segmap = cv2.imread("label/img/"+file.split('.')[0]+'.png')
        cv2.imwrite("v4/label/img/"+file.split('.')[0]+'.png', segmap)
        ...
        for i in range(5):
            ...
            cv2.imwrite(f'v4/origin/img/aug_{i+1}_'+file, images_aug)
            segmaps_aug = segmaps_aug.get_arr()
            segmaps_aug = np.array(segmaps_aug, np.uint8)
            cv2.imwrite(f'v4/label/img/aug_{i+1}_'+file.split('.')[0]+'.png', segmaps_aug)
